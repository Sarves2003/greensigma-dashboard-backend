...
class OverviewV2Service {
    constructor() {
        this.plot5Cache = new Map();
    }
    ...
    buildUserFilter(filters) {
        ...
        if (filters.referralCode)
            f.referalCode = filters.referralCode;
        return f;
    }
    // ============ KEY METRICS ============
    async getKeyMetrics(filters) {
        ...
        const signupIds = signups.map((u) => u._id);
        ...
        const loginQuery = { loginTime: { $gte: startDate, $lt: endDate }, status: 'SUCCESS' };
        if (signupIds.length > 0)
            loginQuery.userId = { $in: signupIds };
    ...
    async getActiveUserCount(startDate, endDate, userIds) {
        const db = (0, database_1.getDatabase)();
        const scope = (q) => (userIds ? { ...q, userId: { $in: userIds } } : q);
    ...
    async getAvgDaysToFirstPortfolio(signupIdsStr, signups) {
        const db = (0, database_1.getDatabase)();
        const deployments = await db.collection('portfolio_details')
            ...
            .sort({ createdAt: 1 })
            .toArray();
        const firstDeployByUser = new Map();
        deployments.forEach((d) => {
            if (!firstDeployByUser.has(d.userId))
                firstDeployByUser.set(d.userId, new Date(d.createdAt));
        });
        const signupDateByUser = new Map(signups.map((u) => [u._id.toString(), new Date(u.createdOn)]));
        const daysList = [];
        firstDeployByUser.forEach((deployDate, userId) => {
            const signupDate = signupDateByUser.get(userId);
            if (signupDate)
                daysList.push((deployDate.getTime() - signupDate.getTime()) / (1000 * 60 * 60 * 24));
        });
    ...
    async getFeatureUsage(filters) {
        ...
        const cohortIdsStr = cohort.map((u) => u._id.toString());
        const empty = [
        ...
        if (cohortIdsStr.length === 0)
            return empty;
        const scoped = (query = {}) => ({ ...query, userId: { $in: cohortIdsStr } });
        const [stockScoreCount, stockScoreUsers, etfScoreCount, etfScoreUsers, intradayCount, intradayUsers, stockBtSuccess, stockBtTotalUsers, etfBtSuccess, etfBtTotalUsers, liveRealPortfolios, brokerConnectedUsers,] = await Promise.all([
        ...
        const avg = (count, uniqueUsers) => uniqueUsers > 0 ? parseFloat((count / uniqueUsers).toFixed(1)) : null;
        return [
    ...
    async getSignupsMonthly(filters) {
        const db = (0, database_1.getDatabase)();
        const userFilter = this.buildUserFilter(filters);
        const { startDate, endDate } = filters.dateRange;
        ...
        return {
            labels: rows.map((r) => monthLabel(r._id)),
            values: rows.map((r) => r.count),
        };
    }
    // ============ PLOT 2: Ledger usage count by signup cohort, summed (obeys global filter) ============
    async getLedgerUsageByCohort(filters, ledgerItems) {
        const db = (0, database_1.getDatabase)();
        const userFilter = this.buildUserFilter(filters);
        const { startDate, endDate } = filters.dateRange;
        ...
        const cohortMap = new Map(); // monthKey -> userIds
        const cohortObjectIds = new Map();
        users.forEach((u) => {
            const key = monthKey(new Date(u.createdOn));
            if (!cohortMap.has(key)) {
                cohortMap.set(key, []);
                cohortObjectIds.set(key, []);
            }
        ...
        await Promise.all(sortedKeys.flatMap(key => {
            ...
            return ledgerItems.map(async (item) => {
                ...
                const query = { userId: { $in: cfg.idField === 'obj' ? userObjectIds : userIdsStr } };
                if (item === 'login')
    ...
    async getActivationRate(monthKeys, type, dayWindow = 30) {
        const db = (0, database_1.getDatabase)();
        const rows = [];
        for (const key of monthKeys) {
            ...
            const signupDateMap = new Map(users.map((u) => [u._id.toString(), new Date(u.createdOn)]));
            const query = { userId: { $in: userIdsStr } };
            if (type === 'real') {
            ...
            portfolios.forEach((p) => {
                ...
                if (days >= 0 && days <= dayWindow)
                    activatedSet.add(p.userId);
            });
            rows.push({
    ...
    async getLiveCapitalRate(monthKeys) {
        ...
        for (const key of monthKeys) {
            ...
            if (cohortUsers.length === 0)
                continue;
            const userIdsStr = cohortUsers.map((u) => u._id.toString());
    ...
    async getMonthlyActivePaid(ledgerItems) {
    ...
    async computeMonthlyActivePaid(ledgerItems) {
        ...
        const activeSets = new Map(months.map(k => [k, new Set()]));
        await Promise.all(months.flatMap(key => {
            ...
            return ledgerItems.map(async (item) => {
                ...
                const query = { userId: { $in: userIdMatch }, [cfg.dateField]: { $gte: mStart, $lt: mEnd } };
                if (item === 'login')
                    query.status = 'SUCCESS';
                const ids = await db.collection(cfg.name).distinct('userId', query);
                const targetSet = activeSets.get(key);
                ids.forEach((id) => targetSet.add(id.toString()));
            });
        }));
        return {
    ...
    async getActiveUserFlow(userType, dayWindow) {
        const db = (0, database_1.getDatabase)();
        const now = new Date();
        ...
        const rollingActiveUsers = new Set();
    ...
    async getActiveUserFlowByPeriod(userType, period) {
        const db = (0, database_1.getDatabase)();
        const now = new Date();
        ...
        const months = monthStarts.map((mStart) => {
            ...
            dayBuckets.forEach((set, dayKey) => {
                ...
                if (d >= mStart && d < mEnd)
                    set.forEach((uid) => monthUsers.add(uid));
            });
    ...
    async getEngagementDistribution(userType, period, customStart, customEnd) {
        const db = (0, database_1.getDatabase)();
        const now = new Date();
        const currentMonthStart = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), 1));
        let rangeStart;
        let rangeEnd; // exclusive
        if (period === 'lastMonth') {
        ...
        await Promise.all(exports.ACTIVE_ACTION_COLLECTIONS.map(async (cfg) => {
            ...
            docs.forEach((doc) => {
                ...
                if (!uid || !rawDate)
                    return;
                const dayKey = new Date(rawDate).toISOString().slice(0, 10);
                if (!userDayMap.has(uid))
                    userDayMap.set(uid, new Set());
    ...
    async getActiveUserBreakdown(userType, granularity, startDate, endDate) {
        const db = (0, database_1.getDatabase)();
...
exports.OverviewV2Service = OverviewV2Service;
...
OverviewV2Service.ENGAGEMENT_BUCKETS = [
...
//# sourceMappingURL=OverviewV2Service.js.map