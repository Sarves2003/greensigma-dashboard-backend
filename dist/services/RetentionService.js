...
const ActivityRepository_2 = require("../repository/ActivityRepository");
const ActivityRepository_3 = require("../repository/ActivityRepository");
const ActivityRepository_4 = require("../repository/ActivityRepository");
const ActivityRepository_5 = require("../repository/ActivityRepository");
...
class RetentionService {
    constructor() {
        ...
        this.backtestRepo = new ActivityRepository_2.BacktestRepository();
        this.etfScoreRepo = new ActivityRepository_3.ETFScoreRepository();
        this.etfBacktestRepo = new ActivityRepository_4.ETFBacktestRepository();
        this.intradayRepo = new ActivityRepository_5.IntradayScoreRepository();
        this.brokerRepo = new BrokerRepository_1.BrokerRepository();
        this.portfolioRepo = new BrokerRepository_2.PortfolioRepository();
    }
    // Get cohort retention heatmap data
    async getCohortRetention(filters, granularity) {
        ...
        const [loginsData, stockScoresData, backtestData, etfScoresData, etfBacktestData, intradayData, brokerData, portfolioData] = await Promise.all([
        ...
        userActivityMap.forEach((activity, userId) => {
            const lastActivityDate = activity.activityDates.length > 0
                ? new Date(Math.max(...activity.activityDates.map((d) => d.getTime())))
                : null;
    ...
    parseCohortKey(cohortKey, granularity) {
        if (granularity === 'monthly') {
    ...
    addPeriods(date, periods, granularity) {
    ...
    async getKPIComparison(filters) {
        const db = (0, database_1.getDatabase)();
    ...
    async getActiveUsersForPeriod(filters, userIds) {
        const db = (0, database_1.getDatabase)();
