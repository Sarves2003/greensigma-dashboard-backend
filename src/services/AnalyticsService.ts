...
import {
  ...
  BacktestRepository,
  ETFScoreRepository,
  ETFBacktestRepository,
  IntradayScoreRepository,
} from '../repository/ActivityRepository';
import { BrokerRepository, PortfolioRepository } from '../repository/BrokerRepository';
...
export class AnalyticsService {
  ...
  private buildUserFilter(filters: FilterOptions): Filter<User> {
    ...
    return userFilter as Filter<User>;
  }

  // Get filtered user IDs for use in activity queries
  private async getFilteredUserIds(filters: FilterOptions): Promise<string[]> {
  ...
  // New Users
  async getNewUsers(filters: FilterOptions): Promise<KPIResponse> {
    const userFilter = this.buildUserFilter(filters);
    userFilter.createdOn = { $gte: filters.dateRange.startDate, $lt: filters.dateRange.endDate } as any;

    const users = await this.userRepo.findMany(userFilter as Filter<User>);
    return {
      value: users.length,
      label: 'New Users',
    };
  ...
  // Active Users
  async getActiveUsers(filters: FilterOptions): Promise<KPIResponse> {
    ...
    if (afterCutoff) {
      const logins = await this.loginRepo.getUniqueLoginUsers(
  ...
  async getSuccessfulLogins(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    const count = await this.loginRepo.countSuccessfulLogins(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  async getFailedLogins(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    const count = await this.loginRepo.countFailedLogins(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  async getLoginSuccessRate(filters: FilterOptions): Promise<KPIResponse> {
    const successful = await this.loginRepo.countSuccessfulLogins(
      filters.dateRange.startDate,
      filters.dateRange.endDate
    );
    const failed = await this.loginRepo.countFailedLogins(
      filters.dateRange.startDate,
      filters.dateRange.endDate
    );
  ...
  async getStockScores(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    ...
    const totalScores = await this.stockScoreRepo.countScoresBetween(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  // Stock Backtests
  async getStockBacktests(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);

    const successful = await this.backtestRepo.countBacktestsByStatus(
      'Success',
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    ...
    const failed = await this.backtestRepo.countBacktestsByStatus(
      'Failed',
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
  ...
  // ETF Scores
  async getETFScores(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    ...
    const totalScores = await this.etfScoreRepo.countScoresBetween(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  // ETF Backtests
  async getETFBacktests(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);

    const successful = await this.etfBacktestRepo.countBacktestsByStatus(
      'Success',
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    ...
    const failed = await this.etfBacktestRepo.countBacktestsByStatus(
      'Failed',
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
  ...
  // Paper Portfolio
  async getPaperPortfolio(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    ...
    const portfolios = await this.portfolioRepo.aggregate<{ count: number }>(
      [
        { $match: matchStage },
        { $count: 'count' },
      ]
  ...
  async getLiveRealPortfolio(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
  ...
  async getBrokerConnected(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    const brokers = await this.brokerRepo.getConnectedUsersBetween(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  async getIntradayScores(filters: FilterOptions): Promise<KPIResponse> {
    const userIds = await this.getFilteredUserIds(filters);
    ...
    const totalScores = await this.intradayRepo.countScoresBetween(
      filters.dateRange.startDate,
      filters.dateRange.endDate,
      userIds
    );
  ...
  async getDailyTrend(filters: FilterOptions): Promise<ChartDataPoint[]> {
    const data = await this.stockScoreRepo.getTrendData(
      filters.dateRange.startDate,
      filters.dateRange.endDate
    );
  ...
  async getUserTypeDistribution(filters: FilterOptions): Promise<ChartDataPoint[]> {
    const types = ['Free', 'Webinar', 'Tribe'];
    const data: ChartDataPoint[] = [];

    for (const type of types) {
      const userFilter = this.buildUserFilter({ ...filters, userType: type });
      const count = await this.userRepo.count(userFilter as Filter<User>);
      data.push({
        date: type,
        value: count,
        label: type,
  ...
  async getStateDistribution(filters: FilterOptions): Promise<ChartDataPoint[]> {
    const userFilter = this.buildUserFilter(filters);
    ...
    for (const state of states) {
      if (state) {
        const stateFilter = { ...userFilter, state } as Filter<User>;
  ...
  async getReferralDistribution(filters: FilterOptions): Promise<ChartDataPoint[]> {
    const userFilter = this.buildUserFilter(filters);
    ...
    for (const code of codes) {
      if (code) {
        const codeFilter = { ...userFilter, referalCode: code } as Filter<User>;
  ...
  async getLivePortfolioUsers(filters: FilterOptions): Promise<User[]> {
    const userIds = await this.getFilteredUserIds(filters);

    const livePortfolios = await this.portfolioRepo.aggregate<{ _id: string }>(
      [
        {
          $match: {
            createdAt: { $gte: filters.dateRange.startDate, $lt: filters.dateRange.endDate },
  ...
  async getPaperPortfolioUsers(filters: FilterOptions): Promise<User[]> {
    const userIds = await this.getFilteredUserIds(filters);

    const paperPortfolios = await this.portfolioRepo.aggregate<{ _id: string }>(
      [
        {
          $match: {
            createdAt: { $gte: filters.dateRange.startDate, $lt: filters.dateRange.endDate },
  ...
  async getStockScoreUsers(filters: FilterOptions): Promise<User[]> {
    const userIds = await this.getFilteredUserIds(filters);

    const scoreUsers = await this.stockScoreRepo.getUniqueUsersBetween(
      filters.dateRange.startDate,
      filters.dateRange.endDate
    );
