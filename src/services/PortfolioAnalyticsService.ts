...
import { FilterOptions, KPIResponse } from '../types';
...
export class PortfolioAnalyticsService {
  ...
  async getPortfolioMetrics(filters: FilterOptions): Promise<any> {
    ...
    const realUserCount = (data.realPortfolioUsers || []).filter((u: any) => u !== null).length;
    const paperUserCount = (data.paperPortfolioUsers || []).filter((u: any) => u !== null).length;
  ...
  async getPortfolioTrend(filters: FilterOptions): Promise<any[]> {
    const userIds = await this.getFilteredUserIds(filters);
  ...
  async getPortfolioTypeBreakdown(filters: FilterOptions): Promise<any[]> {
    const userIds = await this.getFilteredUserIds(filters);
    const brokerTypes = filters.brokerTypes || ['kite', 'zebu', 'paper_trade'];
  ...
  async getTopInvestors(filters: FilterOptions, limit: number = 10, offset: number = 0): Promise<any> {
    const userIds = await this.getFilteredUserIds(filters);
    const brokerTypes = filters.brokerTypes || ['kite', 'zebu'];
    ...
    const investors = await Promise.all(
      paginatedInvestors.map(async (item: any) => {
        let username = 'Unknown';
        let createdOn: Date | null = null;
        try {
          const userId = new ObjectId(item._id);
          const user = await this.userRepo.findOne({ _id: userId } as any);
  ...
  async getUserPortfolios(userId: string, filters: FilterOptions): Promise<any[]> {
