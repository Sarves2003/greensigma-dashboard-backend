...
function classifyLocation(
  ...
  uploadedLocation: string | null | undefined,
  strictChennai: boolean
): 'chennai' | 'non-chennai' | 'unknown' {
  const d = (district || '').trim().toLowerCase();
  if (d === 'chennai') return 'chennai';
  if (!strictChennai && CHENNAI_NEARBY_AREAS.some((area) => d.includes(area))) return 'chennai';
  if (d) return 'non-chennai';

  const u = (uploadedLocation || '').trim().toLowerCase();
  if (u.includes('chennai')) return 'chennai';
  if (!strictChennai && CHENNAI_NEARBY_AREAS.some((area) => u.includes(area))) return 'chennai';
  if (u) return 'non-chennai';
...
export class FunnelAnalysisService {
  ...
  async getWebinarBatchDetail(requestedKeys?: string[], strictChennai = false): Promise<any[]> {
    ...
    const regDatesByPhone = new Map<string, Date[]>();
    const regDatesByEmail = new Map<string, Date[]>();
    for (const r of registrations) {
      if (!r.webinarDate) continue;
      if (r.phone) {
        if (!regDatesByPhone.has(r.phone)) regDatesByPhone.set(r.phone, []);
        regDatesByPhone.get(r.phone)!.push(r.webinarDate);
      }
      if (r.email) {
        if (!regDatesByEmail.has(r.email)) regDatesByEmail.set(r.email, []);
    ...
    for (const u of users as any[]) {
      const phone = normalizePhone(u.mobile || u.whatsappNumber);
      const email = normalizeEmail(u.email);
      if (phone && !userByPhone.has(phone)) userByPhone.set(phone, u);
      if (email && !userByEmail.has(email)) userByEmail.set(email, u);
    }
    ...
    const uploadLocationByPhone = new Map<string, string>();
    const uploadLocationByEmail = new Map<string, string>();
    for (const u of uploads as any[]) {
      if (u.phone && !uploadLocationByPhone.has(u.phone)) uploadLocationByPhone.set(u.phone, u.location);
      if (u.email && !uploadLocationByEmail.has(u.email)) uploadLocationByEmail.set(u.email, u.location);
    }

    const resolveLocation = (phone: string, email: string): 'chennai' | 'non-chennai' | 'unknown' => {
    ...
    return dateKeys.map((key) => {
      ...
      for (const p of paidForBatch) {
        ...
        if (startDate) {
          ...
          if (paidDate) daysToPay.push((paidDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        }
